SELECT
				...
				A.CO_NAVN,
				CAST(RTRIM(dbo.JY72000V.VEJADRNAVN + ' ' + REPLACE(LTRIM(REPLACE(RTRIM(dbo.JY72000V.HUS_NR), '0', ' ')), ' ', '0') + ' ' + RTRIM(LTRIM(A.ETAGE)) + ' ' + RTRIM(LTRIM(A.SIDE_DØRNR))) AS VARCHAR(50)) AS Adresse,
				...
				CASE
					...
					ELSE CAST(LEFT(A.ADR_TILFLYT_TS, 10) AS date)
				END AS Tilflytningsdato,
				CASE
					WHEN A.ADR_FRAFLYT_TS > '4713-12-31'
					THEN CAST('4713-12-31' AS date)
					ELSE CAST(LEFT(A.ADR_FRAFLYT_TS, 10) AS date)
				END AS Fraflytningsdato,
				CASE
					WHEN A.KOM_TILFLYTDATO < '1753-01-01'
					THEN CAST('1753-01-01' AS date)
					ELSE CAST(LEFT(A.KOM_TILFLYTDATO, 10) AS date)
				END AS Kommunetilflytningsdato,
				CASE
					WHEN A.KOM_FRAFLYTDATO < '1753-01-01'
					THEN CAST('1753-01-01' AS date)
					ELSE CAST(LEFT(A.KOM_FRAFLYTDATO, 10) AS date)
				END AS Kommunefraflytningsdato,
				CASE
					WHEN A.CPR_BOSÆT_TS < '1753-01-01'
					THEN CAST('1753-01-01' AS date)
					ELSE CAST(LEFT(A.CPR_BOSÆT_TS, 10) AS date)
				...
				CASE
					...
					THEN CAST('1753-01-01' AS date)
					ELSE CAST(LEFT(B.BORGERSTATUS_TS, 10) AS date)
				...
				CASE
					...
					THEN CAST('1753-01-01' AS date)
					ELSE CAST(LEFT(B.STATSB_RET_TS, 10) AS date)
				END AS Statsborgerret_ændring,
				CASE
					WHEN B.STATBORG_OPHØR_TS > '4713-12-31'
					THEN CAST('4713-12-31' AS date)
					ELSE CAST(LEFT(B.STATBORG_OPHØR_TS, 10) AS date)
